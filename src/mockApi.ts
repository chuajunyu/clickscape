    ...
    return new Promise((resolve, reject) => {
        ...
        req.onupgradeneeded = () => {
            ...
            if (!req.result.objectStoreNames.contains(WORLD_DB_STORE)) {
                req.result.createObjectStore(WORLD_DB_STORE, {
                    keyPath: "world_id",
                });
            }
        };
...
async function getImage(key: string): Promise<string | null> {
    ...
    return new Promise((resolve, reject) => {
        const req = db
            .transaction(IMAGE_DB_STORE, "readonly")
            .objectStore(IMAGE_DB_STORE)
            .get(key);
        req.onsuccess = () =>
...
async function getAllWorldsFromDB(): Promise<World[]> {
    ...
    return new Promise((resolve, reject) => {
        const req = db
            .transaction(WORLD_DB_STORE, "readonly")
            .objectStore(WORLD_DB_STORE)
            .getAll();
        req.onsuccess = () => {
...
async function getWorldFromDB(worldId: string): Promise<World | null> {
    ...
    return new Promise((resolve, reject) => {
        const req = db
            ...
            .get(worldId);
        req.onsuccess = () =>
            resolve(isWorld(req.result) ? normalizeWorld(req.result) : null);
        req.onerror = () => reject(req.error);
    });
}
...
function worldIdFromPrompt(prompt: string): string {
    ...
    return `${hashText(normalized)}${hashText(normalized.slice(0, 32))}`.slice(
        0,
        16,
    );
}
...
function isGraphNode(
    node: WorldNode | LegacyWorldNode | undefined,
): node is WorldNode {
...
function isWorldEdge(edge: string | WorldEdge | undefined): edge is WorldEdge {
    return (
        !!edge && typeof edge === "object" && typeof edge.node_id === "string"
    );
}
...
function readWorldsFromLocalStorage(): World[] {
    try {
        ...
        return Array.isArray(parsed)
            ? parsed.filter(isWorld).map(normalizeWorld)
            : [];
    } catch {
        return [];
    }
...
function normalizeWorld(world: World): World {
    ...
    if (!isGraphNode(origin)) {
        const legacyOrigin = nextWorld.nodes["0,0"] as
            | LegacyWorldNode
            | undefined;
        if (legacyOrigin) {
            nextWorld.nodes[ORIGIN_NODE_ID] = {
                ...
                location:
                    legacyOrigin.location ?? buildOriginLocation(world.prompt),
    ...
    for (const node of Object.values(nextWorld.nodes)) {
        ...
        node.location ||=
            node.legacy?.location ?? buildOriginLocation(world.prompt);
    }
...
function buildChildLocation(
    currentLocation: string,
    targetLabel: string,
): string {
    const parentLocation = compactLocation(currentLocation);
    return compactLocation(
        `inside or beyond ${targetLabel}, within ${parentLocation}`,
    );
}
...
function parseHiddenTargetCheckResult(
    ...
    return {
        ...
        reason:
            reason ||
            (matched
                ? "Scene appears to satisfy the objective."
                : "Scene does not yet satisfy the objective."),
    };
}
...
function loadImage(sourceImageUrl: string): Promise<HTMLImageElement> {
    return new Promise((resolve, reject) => {
        ...
        image.onerror = () =>
            reject(new Error("Failed to load source panorama for crop"));
...
async function analyzeClickTargetWithCrop({
    ...
    const targetCropUrl = await cropClickTargetImage({
        sourceImageUrl,
        pitch,
        yaw,
    });
...
async function generateImageFromReferences({
    ...
    if (!response.ok) {
        throw new Error(
            data.error?.message || "OpenAI referenced image generation failed",
        );
    }
...
async function nodePayload(
    ...
    if (
        !imageUrl &&
        node.legacy?.x !== undefined &&
        node.legacy?.y !== undefined
    ) {
        imageUrl = await getImage(
            legacyImageKey(world.world_id, node.legacy.x, node.legacy.y),
        );
        if (imageUrl)
...
export async function getWorldNode(
    worldId: string,
...
export async function enterTarget({
    ...
    const { targetLabel, targetCropUrl } = await analyzeClickTargetWithCrop({
        ...
        currentLocation:
            currentNode.location || buildOriginLocation(world.prompt),
        sourceImageUrl,
        pitch,
        yaw,
    });
...
export async function generateGoalSet({
    ...
    try {
                ...
                input: [
                    {
                        ...
                        content: [
                            {
                                type: "input_text",
                                text: buildHiddenTargetInstruction({
                                    worldPrompt,
                                    currentContext,
                                    currentLocation,
                                }),
        ...
        if (timeoutController.signal.aborted) {
            throw new Error(
                "Goal generation timed out. Try another scene or prompt.",
            );
        }
        throw error;
    } finally {
...
export async function checkHiddenTargetSatisfied({
            ...
            input: [
                {
                    ...
                    content: [
                        {
                            type: "input_text",
                            text: buildTargetSatisfactionInstruction({
                                hiddenTarget,
                                currentContext,
                                currentLocation,
                            }),
    ...
    if (!response.ok) {
        throw new Error(
            data.error?.message || "OpenAI hidden target verification failed",
        );
    }
    const text = extractResponseText(data);
...
export async function getWorldHistory(): Promise<WorldHistoryResponse> {
    ...
    const summaries = await Promise.all(
        worlds.map(async (world): Promise<WorldSummary> => {
            ...
            let originImage = originNode
                ? await getImage(imageKey(world.world_id, ORIGIN_NODE_ID))
                : null;
            if (
                !originImage &&
                isGraphNode(originNode) &&
                originNode.legacy?.x !== undefined &&
                originNode.legacy?.y !== undefined
            ) {
                originImage = await getImage(
                    legacyImageKey(
                        world.world_id,
                        originNode.legacy.x,
                        originNode.legacy.y,
